...
pub fn __clappen_template_impl(args: TokenStream, target: TokenStream) -> TokenStream {
    ...
    let expanded = clappen_template_impl::template_impl::expand_template_impl(item, attrs);
...
///     - not handled: tag params behind a non-reference wrapper (`Option<Base>`, `Box<Base>`); bare and `&Base` are fine
///
