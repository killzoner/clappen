...
fn collect_nested_fields(
    struct_def: &ItemStruct,
) -> syn::Result<Vec<(TokenStream, String, Ident)>> {
    ...
    for field in &struct_def.fields {
        ...
        let metas =
            attr.parse_args_with(Punctuated::<NestedAttributes, Token![,]>::parse_terminated)?;
...
mod tests {
    ...
    fn collected(struct_def: &ItemStruct) -> Vec<(String, String, String)> {
        collect_nested_fields(struct_def)
            .unwrap()
            .into_iter()
            .map(|(apply, prefix, field)| (apply.to_string(), prefix, field.to_string()))
            .collect()
    }
